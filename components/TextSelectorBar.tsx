...
function TextSelectorBar({ puncChangeFunc, numChangeFunc, capsChangeFunc, puncState, numState, capsState, divColour, textColour, hoverColour, textSelectColour }: SelectorProp) {
  ...
  const textHoverColour = "hover:" + hoverColour;
  const svgHoverColour = "hover:" + "fill-" + hoverColour.slice(5);

  const modifiedOuterDivClass = `flex flex-row justify-center gap-10 w-96 h-8 rounded-md ${divColour}`;
  const modifiedInnerDivClass = `flex flex-row gap-2 h-full items-center justify-center group ${textHoverColour} `;
  const puncColour = `${puncState ? textSelectColour : textColour}`;
  const modifiedSvgClass = `${fillColour} group-hover:fill-dolphin-bright`;

  return (
    <div className={modifiedOuterDivClass}>
      <div className={modifiedInnerDivClass + puncColour}>
        <svg className={modifiedSvgClass} width="20px" height="20px" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
        ...
        <button className="outline-none" onClick={puncChangeFunc}>
          punctuation
        </button>
      </div>
      <div className={modifiedInnerDivClass}>
        <svg className={modifiedSvgClass} width="20px" height="20px" viewBox="0 0 56 56" xmlns="http://www.w3.org/2000/svg">
        ...
        <button className="outline-none" onClick={numChangeFunc}>
          numbers
        </button>
      </div>
      <div className={modifiedInnerDivClass}>
        <svg className={modifiedSvgClass} width="20px" height="20px" viewBox="0 0 32 32" id="icon" xmlns="http://www.w3.org/2000/svg">
        ...
        <button className="outline-none" onClick={capsChangeFunc}>
          caps
        </button>
      </div>
    </div>
