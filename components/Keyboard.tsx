...
type AddClassProp = { addClass?: string; theme: string; funcTheme: string; pressedTheme: string; backgroundTheme: string; textColour: string; textPressedColour: string; themeSelectorOpen: boolean };

function Keyboard({ themeSelectorOpen, addClass, theme, funcTheme, pressedTheme, backgroundTheme, textColour, textPressedColour }: AddClassProp) {
	...
	const modifiedDivClass = `flex flex-row justify-center items-center w-[970px] gap-4 h-[235px] rounded-md transition-opacity duration-200 ${backgroundTheme} ${isKeyboardVisible ? "opacity-100" : "opacity-0"}`;
